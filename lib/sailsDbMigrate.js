...
function parseSailsConfig(sailsConfig) {
  ...
  res.cleanURL = buildURL(connection);
	...
	// Add support for multiple statements for MySQL SQL files
	if ( sailsConfig.migrations.multipleStatements ) {
		res.url += '?multipleStatements='+connection.multipleStatements;
		res.cleanURL += '?multipleStatements='+connection.multipleStatements;
	}
